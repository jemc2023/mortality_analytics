...
DEFAULT_BASE_URL = "https://r9szgxlp-8000.use.devtunnels.ms/"
DBFS_STAGING_DIR = "dbfs:/Workspace/Shared/tmp/who_mortality"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(HTTP_RETRY_COUNT):
        try:
            ...
            last_error = error
            if status_code not in {502, 503, 504} or attempt == HTTP_RETRY_COUNT - 1:
                raise
    ...
    if last_error:
        raise last_error
    raise RuntimeError("Unable to fetch remote file")
